...
typedef NS_ENUM(NSUInteger, XMAsyncTaskState) {
    XMAsyncTaskStatePending = 0,
    XMAsyncTaskStateSucceeded,
    XMAsyncTaskStateErrored,
};

@interface XMAsyncTask ()

@property (readwrite) XMAsyncTaskState state;
...
@property (copy) XMAsyncTaskCancelBlock cancelBlock;
...
@property (strong) XMAsyncTask *task;

- (instancetype)initWithTask:(XMAsyncTask *)task;

@end

@implementation XMAsyncTask {
...
- (instancetype)init {
    ...
    _state = XMAsyncTaskStatePending;
...
+ (instancetype)taskForBlock:(XMAsyncTaskResolveBlock)block {
    XMAsyncTask *task = [self new];
    XMAsyncTaskResolver *resolver = [[XMAsyncTaskResolver alloc] initWithTask:task];
...
+ (instancetype)taskWithResult:(id)result {
    return [self taskForBlock:^XMAsyncTaskCancelBlock(XMAsyncTaskResolver *resolver) {
...
+ (instancetype)when:(NSArray *)tasks {
    return [self taskForBlock:^XMAsyncTaskCancelBlock(XMAsyncTaskResolver *resolver) {
        ...
        void (^cancelRemainingBlock) (void) = ^{
            ...
            for (XMAsyncTask *task in tasksToCancel) {
                [task cancel];
            }
...
- (instancetype)then:(XMAsyncTaskThenBlock)thenBlock {
    return [[self class] taskForBlock:^XMAsyncTaskCancelBlock(XMAsyncTaskResolver *resolver) {
...
- (instancetype)pipe:(XMAsyncTask *(^)(id result))block {
    ...
    return [[self class] taskForBlock:^XMAsyncTaskCancelBlock(XMAsyncTaskResolver *resolver) {
        __block XMAsyncTask *pipedTask = nil;
        ...
        XM_WEAK_SELF weakSelf = self;
        XM_WEAK(pipedTask) weakPipedTask = pipedTask;
...
- (BOOL)completed {
    return self.state != XMAsyncTaskStatePending;
}

- (BOOL)succeeded {
    return self.state == XMAsyncTaskStateSucceeded;
}

- (BOOL)errored {
    return self.state == XMAsyncTaskStateErrored;
}
...
- (void)succeedWithResult:(id)result {
    [self resolveWithState:XMAsyncTaskStateSucceeded result:result];
}

- (void)failWithError:(NSError *)error {
    [self resolveWithState:XMAsyncTaskStateErrored result:error];
}

- (void)notifyProgress:(float)progress {
    for (XMAsyncTaskProgressBlock callback in self.progressCallbacks) {
...
- (void)performSuccessCallbacksWithResult:(id)result {
    for (XMAsyncTaskSuccessBlock callback in self.successCallbacks) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)performErrorCallbacksWithError:(NSError *)error {
    for (XMAsyncTaskErrorBlock callback in self.errorCallbacks) {
        dispatch_async(dispatch_get_main_queue(), ^{
            callback(self.result);
        });
    }
...
- (void)performCompleteCallbacksWithResult:(id)result error:(NSError *)error {
    for (XMAsyncTaskCompleteBlock callback in self.completeCallbacks) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
@implementation XMAsyncTaskResolver
